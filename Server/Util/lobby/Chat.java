	...
	public void run() {
		...
		while(user.getChatState()) {
			...
			if(ChatText.startsWith("/w ")) { 
				...
				try {
					System.out.println("채널넘버"+Channel.getRoomsSize(1));
					for (int i = 0; i < Channel.getRoomsSize(1); i++) {
						user_room = Channel.getRoom(1, i);
						user_room_cli = user_room.getArrayList();
						for (int j = 0; j < user_room_cli.size(); j++) {
							if(user_room_cli.get(j).getNickName().equals(words[0])) {
								Send.sendData(user_room_cli.get(j).getChatOut(),"SYSW "+user.getNickName()+"님으로 부터 받은 메세지 : "+words[1]);
